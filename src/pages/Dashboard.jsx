    ...
    async function load() {
      ...
      if (data) {
        if (data.monthly_quota === 0 && data.topup_credits === 0) {
          setNotification({ type: 'error', msg: '⚠️ You have no credits left. Buy top-up credits or upgrade your plan.' })
        } else if (data.monthly_quota <= 3) {
          setNotification({ type: 'warn', msg: `⚡ Only ${data.monthly_quota} credits left this month.` })
